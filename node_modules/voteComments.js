	...
	return function(error, rows) {	
		...
		*/
		...
		if (g_userVote == 0) {
			...
			reputation.updateOnCommentUpVote(g_commentId, g_userId)
		}
		else {
			//User Dislikes the comment
			vote_dislikes = vote_dislikes + 1;
			reputation.updateOnCommentDownVote(g_commentId, g_userId); 
		}
		...
		dbAccess.runQuery(sqlUpdateVotes, function(error, rows) {
			...
			else { //Successful Vote; Simply go back to the previous page
...
VoteComments.display = function(request, response) 
{
	...
	request.getUser(function(error, user) {
		...
		if (user) {
			...
			//If the user is not logged it, redirect to the dynamic message page
			response.flash.addError('To vote, you must be signed in. Please sign in.');
