...
SendMessage.send = function(request, response) {
	//if (request['method'] == 'POST') {
		...
		request.on('end', function() {
			...
			request.getUser(function(error, user){
				//TODO: more discriptive error messages.
				if(error) {
					util.log("Error getting user in MessageInterface.SendMessage.Send. " + error);
					response.flash.addError('Error: Your message was not sent!');
					response.redirectTo('/messages');
				}
				if(!user) {
					util.log("Could not get user in MessageInterface.SendMessage.Send.");
					response.flash.addError('Error: Your message was not sent!');
					response.redirectTo('/messages');
				}
				else {
					...
					users.getIdFromUsername(qs.tousername, function(error, touserid) {
						if (error) {
							...
							response.flash.addError('Error: Could not get toUserId.');
							response.redirectTo('/messages');
						}
						else {
							// Send the message
							messages.sendMessage(user.id, touserid, qs.subject, qs.body, function(error) {
								if (error) {
									...
									response.flash.addSuccess('Your message has been sent successfully.');
									response.redirectTo('/messages');
								}
							});
						}
